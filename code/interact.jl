import Escher: @api, @d, Behavior, Intent, boolattr
import Compose: absolute_units

@api draggable => (Draggable <: Behavior) begin
    arg(tile::Tile)
    kwarg(name::Symbol=:drags)
    kwarg(disabled::Bool=false)
    kwarg(updateon::AbstractString="move")
...
immutable DragIntent <: Intent
end

Escher.default_intent(::Draggable) = DragIntent()

Escher.interpret(::DragIntent, dict) =
...
immutable ResizeIntent <: Intent
end

Escher.default_intent(::Resizable) = ResizeIntent()

Escher.interpret(::ResizeIntent, dict) =
...
Compose.absolute_units(x::EscherWrapper, t::Compose.Transform, 
u::Compose.UnitBox, b::Compose.AbsoluteBoundingBox) = 
EscherWrapper(x.tile, (t, u, b))
Compose.compose(x::Compose.Context, y::Escher.Tile) = 
compose(context(), escher_wrap(y))

Compose.draw(img::Compose.Patchable, prim::EscherWrapper) =
    Escher.render(prim.tile, Dict(:img => img, :unit_args => 
prim.render_state))
...
Patchwork.(:<<)(l::BroadcastContainer, x) = BroadcastContainer([a<<x 
for a in l.array])
Patchwork.(:&)(l::BroadcastContainer, x) = BroadcastContainer([a&x for 
a in l.array])
...
Escher.render(x::ComposeWrapper, state) = begin
    if Compose.isscalar(x.compose_object)
        draw(state[:img], 
Compose.absolute_units(x.compose_object.primitives[1], 
state[:unit_args]...))
    else
        BroadcastContainer([draw(state[:img], 
Compose.absolute_units(x, state[:unit_args]...)) for x in 
x.compose_object.primitives])
    end
end
...
function main(w)
    ...
    sierpinski(2)
    vbox(
    compose(context(), draggable(circle([0.5,0.3,0.4],[0.5,0.3,0.4], 
[0.05,0.02,0.01])) >>> x)
    , x)
end
